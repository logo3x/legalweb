@php
    ...
    $casesLimit = $plan?->max_cases ?? 3;
    $casesPercent = $casesLimit > 0 ? min(100, round(($casesUsed / $casesLimit) * 100)) : 0;
    $casesRemaining = max(0, $casesLimit - $casesUsed);
    $nextPlan = \App\Models\Plan::where('sort_order', '>', ($plan?->sort_order ?? 1))->where('is_active', true)->orderBy('sort_order')->first();
    $nextPrice = $nextPlan ? '$' . number_format($nextPlan->price_monthly, 0, ',', '.') : null;
...
<x-filament-widgets::widget>
    <div style="background: #fff; border-radius: 12px; border: 1px solid #e5e7eb; padding: 20px 24px; font-family: Inter, system-ui, sans-serif;">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                    ...
                    <div style="font-size: 13px; color: #6b7280;">{{ $casesRemaining }} casos disponibles</div>
                </div>
            </div>
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 16px;">
            <span style="font-size: 13px; color: #374151;"><strong>{{ $casesUsed }}</strong> usados</span>
            <span style="font-size: 13px; color: #374151;"><strong>{{ $casesLimit }}</strong> total</span>
        </div>

        {{-- Features badges --}}
        <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 8px; padding: 12px 16px; background: #f0fdf4; border-radius: 8px;">
            @php
                $features = [
